(function () {
  ...
  async function onDeleteSelected() {
    ...
    const confirmed = window.confirm(
      `Delete ${selectedChats.size} selected chat${selectedChats.size === 1 ? "" : "s"}? This hides them from your ChatGPT history and cannot be undone from this tool.`
    );
    if (!confirmed) {
      return;
    }
    ...
    if (failures.length) {
      ...
      console.warn("[ChatSweep] Failed deletions:", failures);
    } else {
